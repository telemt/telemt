...
mod tests {
    ...
    fn user_auth_snapshot_entry_by_id() {
        ...
        let snap = UserAuthSnapshot::from_users(&users).unwrap();
        // HashMap iteration order is non-deterministic — look up by name, not by index.
        let alice_id = snap.user_id_by_name("alice").unwrap();
        let bob_id = snap.user_id_by_name("bob").unwrap();
        assert_ne!(alice_id, bob_id);
        assert_eq!(snap.entry_by_id(alice_id).unwrap().user, "alice");
        assert_eq!(snap.entry_by_id(bob_id).unwrap().user, "bob");
