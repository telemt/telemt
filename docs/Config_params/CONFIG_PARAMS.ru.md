...
## include
  ...
  - **Пример**:
    ...
    ```
  - **Взаимодействие с API**: config API учитывает `include`. `GET /v1/config` и users API разворачивают включения перед разбором, а запись (`PATCH /v1/config` и семейство `POST/PATCH/DELETE /v1/users`) обновляет тот файл, где реально определена редактируемая секция — например, таблица `[access.users]`, вынесенная во включённый `users.toml`, правится на месте именно там, а не дублируется в основной конфиг. Ревизия для optimistic concurrency (`If-Match`) считается по полностью развёрнутому конфигу, поэтому правка включённого файла меняет ревизию. Секция, определённая более чем в одном файле, считается повреждённой и не может редактироваться через API.
## show_link
