...
## Ошибка "Telegram handshake timeout"
Чаще всего встречается у мобильных клиентов Telegram (Android/iOS) в российских сетях, тогда как десктопные клиенты на том же прокси продолжают работать. Причина — устаревший TLS-отпечаток клиента: см. раздел «Распознаваемость для DPI и сканеров» выше и обновите Telegram из официальных источников.

Увеличение `timeouts.client_handshake` не помогает: сервер ждёт байт рукопожатия, которые старый клиент попросту не присылает в ожидаемом виде.
